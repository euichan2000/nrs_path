...
nrs_ver2::Waypoints waypoints_msg;
...
nrs_ver2::Waypoints convertToWaypoints(const std::vector<geometry_msgs::Point> &points, const Triangle_mesh tmesh);
...
int main(int argc, char **argv)
{
    ...
    waypoints_pub = nh.advertise<nrs_ver2::Waypoints>("waypoints_with_normals", 10);
    // controlpoints_pub = nh.advertise<std_msgs::Float64MultiArray>("control_points", 10);

    std::string stl_file_path;
    nh.getParam("path_generator_node/stl_file", stl_file_path);

    // 파일 이름 설정 (웨이포인트와 컨트롤 포인트가 하나의 파일에 저장됨)
    // std::string filename = "/home/nrs_vision/catkin_ws/src/nrs_ver2/data/waypoints_and_controlpoints.txt";

    std::ifstream input(stl_file_path, std::ios::binary);
