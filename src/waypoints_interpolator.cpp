...
nrs_ver2::Waypoints convertToWaypoints(const std::vector<geometry_msgs::Point> &points, const std::vector<geometry_msgs::Point> &reference_points, const Triangle_mesh &mesh, int option);

void clearFile(const std::string &file_path);

void saveWayPointsTOFile(const nrs_ver2::Waypoints &waypoints, const std::string &filename, double force);
...
void waypointsCallback(const nrs_ver2::Waypoints::ConstPtr &msg, ros::Publisher &pub, double desired_interval, const Triangle_mesh &mesh, ros::Publisher &file_pub, ros::NodeHandle &nh);
...
int main(int argc, char **argv)
{
    ...
    ros::init(argc, argv, "waypoints_interpolator_node");
    ros::NodeHandle nh;
    ...
    if (!nh.getParam("waypoints_interpolator_node/stl_file", source_file))
    {
    ...
    // 웨이포인트 구독 및 콜백 함수 설정
    ros::Subscriber waypoints_sub = nh.subscribe<nrs_ver2::Waypoints>("waypoints_with_normals", 10,
                                                                      boost::bind(waypointsCallback, _1, boost::ref(interpolated_waypoints_pub), desired_interval, boost::cref(mesh), boost::ref(file_pub), boost::ref(nh)));
...
void waypointsCallback(const nrs_ver2::Waypoints::ConstPtr &msg, ros::Publisher &pub, double desired_interval, const Triangle_mesh &mesh, ros::Publisher &file_pub, ros::NodeHandle &nh)
{
    ...
    nrs_ver2::Waypoints visual_approach_waypoints = convertToWaypoints(visual_approach_interpolated, original_points, mesh, 1);

    // 두 번째 구간의 quaternion 설정
    nrs_ver2::Waypoints visual_original_waypoints = convertToWaypoints(original_points, original_points, mesh, 2);

    // 세 번째 구간의 quaternion 설정
    nrs_ver2::Waypoints visual_retreat_waypoints = convertToWaypoints(visual_retreat_interpolated, original_points, mesh, 3);

    nrs_ver2::Waypoints visual_final_waypoints;
   ...
   //------------------------------------------------------------------------------------------------------------------
    ...
    // 첫 번째 구간의 quaternion 설정
    nrs_ver2::Waypoints control_approach_waypoints = convertToWaypoints(control_approach_interpolated, control_original_interpolated, mesh, 1);
    // 두 번째 구간의 quaternion 설정
    nrs_ver2::Waypoints control_original_waypoints = convertToWaypoints(original_points, original_points, mesh, 2);

    // nrs_ver2::Waypoints control_original_test_waypoints = convertToWaypoints(original_points, original_points, mesh, 5);
