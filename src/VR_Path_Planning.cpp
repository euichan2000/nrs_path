...
void publishAxis(const geometry_msgs::Point& position, const Kernel::Vector_3& x_axis, const Kernel::Vector_3& y_axis, const Kernel::Vector_3& z_axis, int id)
{
    ...
    x_marker.scale.x = y_marker.scale.x = z_marker.scale.x = 0.001;
    x_marker.scale.y = y_marker.scale.y = z_marker.scale.y = 0.001;
    x_marker.scale.z = y_marker.scale.z = z_marker.scale.z = 0.001;
...
void visualizePath()
{
    ...
    for (auto it = complete_path.rbegin(); it != complete_path.rend(); ++it)
    {
    ...
    for (size_t i = 1; i < line_strip.points.size(); i += 1)
    {
        geometry_msgs::Point point = line_strip.points[i];
        geometry_msgs::Point prev_point = line_strip.points[i - 1];

        face_descriptor face;
        Surface_mesh_shortest_path::Barycentric_coordinates location;
        Point_3 p(point.x, point.y, point.z);
        locate_face_and_point(p, face, location);

        Kernel::Vector_3 normal = CGAL::Polygon_mesh_processing::compute_face_normal(face, tmesh);
        Kernel::Vector_3 direction(prev_point.x - point.x, prev_point.y - point.y, prev_point.z - point.z);
        direction = direction / std::sqrt(direction.squared_length());
        Kernel::Vector_3 y_direction = CGAL::cross_product(normal, direction);

        publishAxis(point, direction, y_direction, normal, i);
    }
}
...
void poseCallback(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
    ...
    pointCallback(boost::make_shared<geometry_msgs::PointStamped>(point_msg));
}
...
int main(int argc, char** argv)
{
    ...
    ros::Subscriber sub = n.subscribe("/clicked_point", 10, pointCallback);
